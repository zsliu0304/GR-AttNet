...
def save_results(rgb_img, grasp_q_img, grasp_angle_img, depth_img=None, no_grasps=1, grasp_width_img=None):
    ...
    def save_plot(img, title, cmap=None, vmin=None, vmax=None, grasps=None):
        ...
        ax.set_title(title)
        ax.axis('off')
        fig.savefig(f'results/{title.lower()}.png')
        plt.close(fig)

    save_plot(rgb_img, 'RGB')
    if depth_img is not None:
        save_plot(depth_img, 'Depth', cmap='gray')
    save_plot(rgb_img, 'Grasp', grasps=gs)
    save_plot(grasp_q_img, 'Quality', cmap='jet', vmin=0, vmax=1)
    save_plot(grasp_angle_img, 'Angle', cmap='hsv', vmin=-np.pi / 2, vmax=np.pi / 2)
    if grasp_width_img is not None:
        save_plot(grasp_width_img, 'Width', cmap='jet', vmin=0, vmax=100)